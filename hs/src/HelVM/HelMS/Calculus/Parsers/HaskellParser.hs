module HelVM.HelMS.Calculus.Parsers.HaskellParser (
  parseCalculus,
  parseLineCalculus,
) where
...
defineCombinatorParser = liftA2 Com ((hash *> identifierParser <* equals) <?> "define combinator") lambdaParser

functionParser :: Parser Lambda
functionParser = liftA2 Abs ((backslash *> (identifierParser <|> underscore) <* arrow) <?> "function") lambdaParser
