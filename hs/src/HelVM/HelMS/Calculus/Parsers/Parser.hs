module HelVM.HelMS.Calculus.Parsers.Parser (
  parseCalculus,
  parseLineCalculus,
) where
...
defineCombinatorParser = liftA2 Com ((colon *> identifierParser) <?> "define combinator") lambdaParser

functionParser :: Parser Lambda
functionParser = liftA2 Abs ((backslash *> (identifierParser <|> underscore) <* dot) <?> "function") lambdaParser
